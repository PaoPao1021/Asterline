...
//! text before the cursor. Pure logic so it is fully unit-tested; the TUI popup
//! and the desktop GUI both render and navigate the result.

use std::collections::HashMap;

use crate::contract::{COMMAND_CATALOG, MODE_OPTIONS};
    ...
    if head.starts_with('/') {
        return match chars.iter().position(|c| c.is_whitespace()) {
            ...
            None => {
                ...
                let items: Vec<CompletionItem> = COMMAND_CATALOG
                    .iter()
                    .filter(|spec| slash_command_matches(spec.name, &lower))
                    .map(|spec| CompletionItem {
                        label: format!("/{} — {}", spec.name, spec.hint),
                        // `/mode` without an argument opens its required
                        // picker, so keep the completed command submit-ready.
                        insert: if spec.name == "mode" {
                            format!("/{}", spec.name)
                        } else {
                            format!("/{} ", spec.name)
                        },
